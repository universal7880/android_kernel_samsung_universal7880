...
struct asv_tbl_info {
	unsigned cpucl01_asv_group:4;
	int cpucl01_modified_group:4;
	unsigned cpucl01_ssa10:2;
	unsigned cpucl01_ssa11:2;
	unsigned cpucl01_ssa0:4;
...
//	unsigned reserverd_0:16;
	unsigned cpucl01_vthr:2;
	unsigned cpucl01_delta:2;
...
*/
	...
	unsigned usbboot_test:1;
	unsigned usbboot_test_result:1;
	unsigned reserved_10:30;
};
#define ASV_INFO_ADDR_BASE	(0x100D9000)
#define ASV_INFO_ADDR_CNT	(sizeof(struct asv_tbl_info) / 4)

struct dram_vendor_margin_t {
	unsigned int vendor_margin[5];
	/* [0]SEC [1]MICRON [2]HYNIX [3][4]ETC */
};
...
static void asv_set_ssa0(unsigned int id, unsigned int ssa0)
{
	...
	case cal_asv_dvfs_cpucl0:
		asv_tbl_info.cpucl01_ssa0 = ssa0;
		break;
...
static void asv_set_ssa1(unsigned int id, unsigned int gnum, unsigned int ssa1)
{
	...
	case cal_asv_dvfs_cpucl0:
		if (gnum == 0)
			asv_tbl_info.cpucl01_ssa10 = ssa1;
		else
			asv_tbl_info.cpucl01_ssa11 = ssa1;
		break;
	case cal_asv_dvfs_cpucl1:
		if (gnum == 0)
			asv_tbl_info.cpucl01_ssa10 = ssa1;
...
//	asv_dvfs_cpucl1->table->max_freq = 1794000;
//	asv_dvfs_g3d->table->max_freq = 962000;
...
#if 1
	if (!asv_tbl_info.fused_grp) {
		asv_tbl_info.cpucl01_asv_group = 0;
		asv_tbl_info.cpucl01_modified_group = 0;
		asv_tbl_info.cpucl01_ssa10 = 0;
		asv_tbl_info.cpucl01_ssa11 = 0;
		asv_tbl_info.cpucl01_ssa0 = 0;
...
//		asv_tbl_info.reserverd_0 = 0;
		asv_tbl_info.cpucl01_vthr = 0;
		asv_tbl_info.cpucl01_delta = 0;
...
*/
		...
		asv_tbl_info.usbboot_test = 0;
		asv_tbl_info.usbboot_test_result = 0;
...
static int get_asv_group(enum dvfs_id domain, unsigned int lv)
{
	...
	switch (domain) {
	case cal_asv_dvfs_cpucl0:
		asv = asv_tbl_info.cpucl01_asv_group;
		mod = asv_tbl_info.cpucl01_modified_group;
		break;
	case cal_asv_dvfs_cpucl1:
		asv = asv_tbl_info.cpucl01_asv_group; //CPUCL0/1 use same ASV table
		mod = asv_tbl_info.cpucl01_modified_group;
		break;
	case cal_asv_dvfs_g3d:
...
static unsigned int get_asv_voltage(enum dvfs_id domain, unsigned int lv)
{
	...
	case cal_asv_dvfs_cpucl0:
		...
		ssa10 = asv_tbl_info.cpucl01_ssa10;
		ssa11 = asv_tbl_info.cpucl01_ssa11;
		ssa0 = asv_tbl_info.cpucl01_ssa0;
...
//		BUG();
	...
	if (strcmp(dfs->vclk.name,"dvfs_mif") == 0) {
		handle = ect_get_block("TIMING");
		if (handle == NULL)
			return;

		size = ect_timing_param_get_size(handle, 0xffff); //load MIF margin table
		if (size == NULL)
			return;

		loc = (void *)size->timing_parameter;

		mif_margin = loc;
		if (mif_margin == NULL)
			return;

		vendor_id = (pwrcal_readl(PMU_DREX_CALIBRATION3)) & 0xff;

		if (vendor_id == 0x1) {
			margin_idx = 0x0;
		} else if (vendor_id == 0xff) {
			margin_idx = 0x1;
		} else if (vendor_id == 0x6) {
			margin_idx = 0x2;
		}
	}
	...
	for (i = 0; i < domain->num_of_table; ++i) {
		...
		for (j = 0; j < domain->num_of_level; ++j) {
			...
			for (k = 0; k < domain->num_of_group; ++k) {
				...
				if (mif_margin != NULL)
					asv_entry->voltage[k] += mif_margin[j].vendor_margin[margin_idx];
			}
		}
	}
}
...
void asv_print_info(void)
{
	...
	pr_info("cpucl0_asv_group : %d\n", asv_tbl_info.cpucl01_asv_group);
...
int asv_ids_information(enum ids_info id)
{
	...
	switch (id) {
		case table_ver:
			...
			break;
		case cpu_asv:
			res = asv_tbl_info.cpucl01_asv_group;
			break;
		case g3d_asv:
